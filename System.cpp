...
unsigned System::check_avl(const MyString& bank_name, unsigned int account_number) const
{
	try {
		...
		if (indOfBank < 0) {
			throw std::runtime_error("There is no bank with that name");
		}
		...
		if (indOfClient < 0) {
			throw std::runtime_error("There is no user with that name in the bank");
		}
		int indOfAccount = banks_container[indOfBank].clientsOfBank[indOfClient].hasAccountWithThatNumber(account_number);
		if (indOfAccount < 0) {
			throw std::runtime_error("The user does not have account with that number");
		}
		else {
...
Bank& System::getBankWhereEmployeeIsWorking(const Employee* e) 
{
	...
	throw std::runtime_error("There is no employee with that name");
}
...
void System::theCheckHasBeenUsed(const Check& check)
{
	...
	if (ind < 0) {
		throw std::runtime_error("You cannot use a check that does not exist");
	}
...
void System::useCheck(const MyString bank_name, unsigned accountNumber, const MyString code)
{
	...
	int ind = returnIndexOfCheck(check);
	if (ind < 0) {
		throw  std::runtime_error("This check does not exist");
	}
	int indOfBank = returnTheIndexOfTheBankWithThatName(bank_name);
	if (indOfBank < 0) {
		throw std::runtime_error("This bank does noe exist");
	}
...
void System::whoami() const
{
	try {
		if (!currentUser) {
			throw std::runtime_error("You need to be loged in the system to check your info");
		} else {
...
void System::exit()
{
	try {
		if (!currentUser) {
			throw std::runtime_error("You need to be loged in the system to exit");
		}
		else {
			currentUser = nullptr;
			std::ofstream ofs("userDataBase.txt");
			if (!ofs.is_open()) {
				throw std::runtime_error("Problem with openning of the file!");
			}
